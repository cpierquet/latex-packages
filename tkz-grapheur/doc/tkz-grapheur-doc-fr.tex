...
\subsubsection{Graduations automatiques (\og nice numbers \fg{})}\label{gradsauto}

Plutôt que d'indiquer manuellement le pas des grilles (\MontreCode{Xgrille}/\MontreCode{Ygrille}), il est possible de laisser le package calculer un pas \og propre \fg{} (1, 2, 5 ou 10 fois une puissance de 10) à partir d'un nombre de graduations visé, sur le principe des algorithmes dits \textit{nice numbers}.

\begin{tcblisting}{listing engine=minted,minted language=latex,colframe=lightgray,colback=lightgray!5,listing only}
\begin{GraphiqueTikz}%
  [clés globales]%
  <Xmin=...,Xmax=...,Ymin=...,Ymax=...,GradsAuto,NbgradsX=...,NbgradsY=...>
  ...
\end{GraphiqueTikz}
\end{tcblisting}

Les clés concernées sont :

\begin{itemize}
  \item \MontreCode{GradsAuto} : booléen (\MontreCode{false} par défaut) qui active le calcul automatique du pas ;
  \item \MontreCode{NbgradsX} : nombre de graduations \textit{visé} en abscisses (\MontreCode{8} par défaut) ;
  \item \MontreCode{NbgradsY} : nombre de graduations \textit{visé} en ordonnées (\MontreCode{6} par défaut).
\end{itemize}

Lorsque \MontreCode{GradsAuto} est actif, les clés \MontreCode{Xgrille}/\MontreCode{Ygrille} (grille principale) sont recalculées à partir de \MontreCode{Xmin}/\MontreCode{Xmax} et \MontreCode{Ymin}/\MontreCode{Ymax}, et les bornes elles-mêmes sont \textit{ajustées} (arrondies au pas le plus proche, vers l'extérieur) afin que la fenêtre reste cohérente avec la grille obtenue. La grille secondaire (\MontreCode{Xgrilles}/\MontreCode{Ygrilles}) est alors fixée automatiquement à la moitié du pas principal.

\smallskip

Ce calcul reste \textit{indicatif} : le nombre de graduations obtenu au final est proche de \MontreCode{NbgradsX}/\MontreCode{NbgradsY} mais pas nécessairement exact, l'objectif étant d'obtenir un pas \og lisible \fg{} plutôt qu'un nombre de graduations exact.

\begin{tcblisting}{listing engine=minted,minted language=latex,colframe=lightgray,colback=lightgray!5}
\begin{GraphiqueTikz}%
  <Xmin=-7,Xmax=138,Ymin=-12,Ymax=57,%
  GradsAuto,NbgradsX=10,NbgradsY=8,%
  Hauteur=7cm,Largeur=10cm>
...
\begin{tcblisting}{listing engine=minted,minted language=latex,colframe=lightgray,colback=lightgray!5}
\begin{GraphiqueTikz}%
  <Xmin=0,Xmax=1,Ymin=0,Ymax=0.0325,%
  GradsAuto,NbgradsX=5,NbgradsY=10,%
  Largeur=12cm,Hauteur=6cm>
  \TracerAxesGrilles%
    [Dernier,Elargir=2.5mm,Police=\small,Format=num]%
    {auto}{auto}
\end{GraphiqueTikz}
\end{tcblisting}

À noter que \MontreCode{GradsAuto} prend le pas sur toute valeur de \MontreCode{Xgrille}/\MontreCode{Ygrille} indiquée par ailleurs : les deux réglages ne sont pas destinés à être combinés.
...
\begin{itemize}
  \item \MontreCode{Methode} : méthode, parmi \MontreCode{fonction/interpo/spline/path} pour les calculs (\MontreCode{fonction} par défaut) ;
  \item \MontreCode{Debut} : début du tracé (\MontreCode{\textbackslash pflxmin} par défaut) ;
  \item \MontreCode{Fin} : fin du tracé (\MontreCode{\textbackslash pflxmax} par défaut) ;
...
\subsection{Intégrales (version améliorée)}

\subsubsection{Version n°1}\label{integr}
...
\subsubsection{Version n°2}\label{integrnew}

Une version alternative, basée sur les \MontreCode{name path} créés par \MontreCode{\textbackslash DefinirCourbe[Nom=...]}, \MontreCode{\textbackslash DefinirCourbeSpline[Nom=...]} ou \MontreCode{\textbackslash DefinirCourbeInterpo[Nom=...]}, permet de s'affranchir de la distinction entre les types de courbes.
...
\begin{itemize}
  \item représenter une intégrale \textbf{sous} une courbe nommée ;
  \item représenter une intégrale \textbf{entre} deux courbes nommées.
\end{itemize}

\smallskip

{\small\faBomb} Cette commande nécessite la librairie \MontreCode{spath3} (\MontreCode{\textbackslash usetikzlibrary\{spath3\}}) chargée par le package.

\smallskip

{\small\faBomb} Les bornes sont données en \textbf{abscisses réelles} ou en \textbf{nœuds TikZ}.

\begin{tcblisting}{listing engine=minted,minted language=latex,colframe=lightgray,colback=lightgray!5,listing only}
%dans l'environnement GraphiqueTikz
%sous une courbe
\RepresenterIntegrale[clés]{path1}{a}{b}
%entre deux courbes
\RepresenterIntegrale[clés]{path1}[path2]{a}{b}
\end{tcblisting}
...
Le premier argument obligatoire est le \textbf{nom} du \MontreCode{name path} de la courbe principale (supérieure dans le cas de deux courbes).

\smallskip

L'argument optionnel suivant est le \textbf{nom} du \MontreCode{name path} de la courbe secondaire (inférieure).

\smallskip

Les deux derniers arguments obligatoires sont les \textbf{bornes} de l'intégrale en abscisses réelles ou en nœuds TikZ.

\smallskip

Dans le cas de deux courbes, le remplissage est géré automatiquement quel que soit l'ordre des courbes sur l'intervalle — pas besoin de préciser laquelle est au-dessus.

\begin{tcblisting}{listing engine=minted,minted language=latex,colframe=lightgray,colback=lightgray!5}
\begin{GraphiqueTikz}%
  [x=1.2cm,y=1cm,Xmin=-5,Xmax=5,Ymin=-2,Ymax=5]
  \TracerAxesGrilles[Grads=false,Elargir=2.5mm]{}{}
  \DefinirCourbe[Nom=cf,Trace=false]<f>%
    {-0.2*(x+4)*(x-4)*(x-1)+1}
  % sous la courbe
  \RepresenterIntegrale[Couleurs=blue/cyan!40]{cf}{-3}{0}
  % hachures
  \RepresenterIntegrale[Couleurs=purple,Style=hachures]{cf}{1}{4}
  \TracerCourbe[Couleur=red]{f(x)}
  \TracerAxesGrilles[Grille=false,Elargir=2.5mm]%
    {-5,-4,...,5}{-2,-1,...,5}
\end{GraphiqueTikz}
...
\begin{GraphiqueTikz}%
  [x=1.2cm,y=1cm,Xmin=-5,Xmax=5,Ymin=-2,Ymax=5]
  \TracerAxesGrilles[Grads=false,Elargir=2.5mm]{}{}
  \DefinirCourbe[Nom=cf,Trace=false]<f>{-0.2*(x+4)*(x-4)*(x-1)+1}
  \DefinirCourbe[Nom=cg,Trace=false]<g>{0.4*x+1}
  % trouver les intersections
  \TrouverIntersections[Nom=ITSC,Aff=false]{cf}{cg}
  % entre deux courbes avec nœuds comme bornes
  \RepresenterIntegrale[Couleurs=olive/yellow!50,Bornes=noeuds,Bord=false]%
    {cf}[cg]{(ITSC-1)}{(ITSC-2)}
  \TracerCourbe[Couleur=red]{f(x)}
...
\begin{GraphiqueTikz}%
  [x=1.2cm,y=1cm,Xmin=-5,Xmax=5,Ymin=-2,Ymax=5]
  \TracerAxesGrilles[Grads=false,Elargir=2.5mm]{}{}
  \def\LISTEST{-5/3/1§-2/4/0§0/2/-1§2/-1/0§4/1/1}
  \DefinirCourbeSpline[Nom=maspline,Trace=false]{\LISTEST}
  \DefinirCourbe[Nom=cg,Trace=false]<g>{0.4*x+1}
  \TrouverIntersections[Nom=ITT,Aff=false]{maspline}{cg}
  % entre spline et courbe
  \RepresenterIntegrale[Couleurs=teal/teal!30,Bornes=abs/noeud,Bord=false]%
    {maspline}[cg]{-4}{(ITT-1)}
  \TracerCourbeSpline[Couleur=red]{\LISTEST}
  \TracerCourbe[Couleur=blue]{g(x)}
  \TracerAxesGrilles[Grille=false,Elargir=2.5mm]%
    {-5,-4,...,5}{-2,-1,...,5}
\end{GraphiqueTikz}
...
% u(n+1) = f(u(n)) avec f -> f(x)
\TracerNuageSuiteF(*)[clés,Debut=...]{u0}{formule xint en x}

% u(n+1) = f(u(n), n) avec f -> f(x, n)
\TracerNuageSuiteFn(*)[clés,Debut=...]{u0}{formule xint en (x, n)}

% u(n+2) = f(u(n+1), u(n)) avec f -> f(x, y)
\TracerNuageSuiteDouble(*)[clés,Debut=...]{u0}[u1]{formule xint en (x, y)}

% u(n+2) = f(u(n+1), u(n), n) avec f -> f(x, y, n)
\TracerNuageSuiteDoubleN(*)[clés,Debut=...]{u0}[u1]{formule xint en (x, y, n)}

% mode automatique (expérimental)
% [Type = Recurrent
%         RecurrentN
%         Explicite
%         RecurrentDouble
%         RecurrentDoubleN ]
\TracerNuageSuiteAuto(*)[Type=...,clés]{u0}[u1]{formule xint}
\end{tcblisting}
...
\begin{GraphiqueTikz}%
  [%
    x=0.5cm,y=0.25cm,Xmin=0,Xmax=30,Xgrille=2,Xgrilles=0.5,%
    Ymin=0,Ymax=20,Ygrille=2,Ygrilles=1%
  ]
  \TracerAxesGrilles{auto}{auto}
  \TracerNuageSuiteAuto*%
  [Type=RecurrentN,Debut=0,Fin=30,CouleurNuage=teal]%
  {20}{0.965*x+0.35+0.07*exp(-0.1*n)}
\end{GraphiqueTikz}
\end{tcblisting}
...
\begin{GraphiqueTikz}%
  [%
    x=0.5cm, y=0.05cm,
    Xmin=0, Xmax=11, Xgrille=1, Xgrilles=1,
    Ymin=0, Ymax=95, Ygrille=10, Ygrilles=5%
  ]
  \TracerAxesGrilles[Elargir=2.5mm, Police=\small]%
    {auto}%
    {auto}%
  \TracerNuageSuiteAuto*%
    [%
      Type=RecurrentDouble,Debut=0,Fin=10,%
      CouleurNuage=red,Style=o,TaillePts=2pt%
    ]%
    {1}[1]{x+y}
\end{GraphiqueTikz}
\end{tcblisting}
...
\begin{GraphiqueTikz}%
  [%
    x=0.35cm, y=0.0045cm,
    Xmin=0, Xmax=32, Xgrille=5, Xgrilles=1,
    Origy=900, Ymin=900, Ymax=1900, Ygrille=200, Ygrilles=100%
  ]
  \TracerAxesGrilles[Elargir=2.5mm, Police=\small]%
    {0,5,...,30}%
    {1000,1200,...,1800}%
  \TracerNuageSuiteAuto*%
    [%
      Type=RecurrentDoubleN,Debut=0,%
      Fin=30,CouleurNuage=teal,Style=x,TaillePts=2pt%
    ]%
    {1000}[1000]{1.02*y-0.01*x+100/(n+1)}
\end{GraphiqueTikz}
\end{tcblisting}
...
\subsection{Points d'intérêt}\label{pointsinteret}

La commande \MontreCode{\textbackslash TrouverPointsInteret} est un \textit{wrapper} qui combine en une seule commande la recherche des \textbf{racines}, du \textbf{maximum global} et du \textbf{minimum global} d'une courbe définie via un \MontreCode{name path}.

\smallskip

Elle est compatible avec les trois types de courbes du package (\MontreCode{\textbackslash DefinirCourbe}, \MontreCode{\textbackslash DefinirCourbeSpline}, \MontreCode{\textbackslash DefinirCourbeInterpo}), à condition d'avoir utilisé la clé \MontreCode{Nom=...} lors de la définition et la clé \MontreCode{Methode=path} pour \MontreCode{\textbackslash TrouverMaximum}/\MontreCode{\textbackslash TrouverMinimum}.

\smallskip

{\small\faBomb} Les \textit{limitations} sont les mêmes que pour \MontreCode{\textbackslash TrouverMaximum}/\MontreCode{\textbackslash TrouverMinimum} :

\begin{itemize}
  \item pas de gestion d'extremums multiples (seul le global sera traité) ;
  \item pas de gestion d'extremums aux bornes du tracé ;
  \item le temps de compilation peut être plus long.
\end{itemize}

\begin{tcblisting}{listing engine=minted,minted language=latex,colframe=lightgray,colback=lightgray!5,listing only}
%dans l'environnement GraphiqueTikz
\TrouverPointsInteret[clés]{nom path}
\end{tcblisting}

Les \MontreCode{[clés]}, optionnelles, disponibles sont :

\begin{itemize}
  \item \MontreCode{Couleur} : couleur globale des points (\MontreCode{black} par défaut) ;
  \item \MontreCode{CouleurRacines} : couleur des racines (défaut → \MontreCode{Couleur}) ;
  \item \MontreCode{CouleurMax} : couleur du maximum (défaut → \MontreCode{Couleur}) ;
  \item \MontreCode{CouleurMin} : couleur du minimum (défaut → \MontreCode{Couleur}) ;
  \item \MontreCode{Style} : style global des points (\MontreCode{o} par défaut) ;
  \item \MontreCode{StyleRacines} : style des racines (défaut → \MontreCode{Style}) ;
  \item \MontreCode{StyleMax} : style du maximum (défaut → \MontreCode{Style}) ;
  \item \MontreCode{StyleMin} : style du minimum (défaut → \MontreCode{Style}) ;
  \item \MontreCode{Racines} : booléen (\MontreCode{true} par défaut) pour afficher les racines ;
  \item \MontreCode{Maximum} : booléen (\MontreCode{true} par défaut) pour afficher le maximum ;
  \item \MontreCode{Minimum} : booléen (\MontreCode{true} par défaut) pour afficher le minimum ;
  \item \MontreCode{NomMax} : nom du nœud créé pour le maximum (\MontreCode{c-max} par défaut) ;
  \item \MontreCode{NomMin} : nom du nœud créé pour le minimum (\MontreCode{c-min} par défaut) ;
  \item \MontreCode{NomRacines} : préfixe des nœuds créés pour les racines (\MontreCode{c-rac} par défaut) ;
  \item \MontreCode{Debut} : début de la recherche (\MontreCode{\textbackslash pflxmin} par défaut) ;
  \item \MontreCode{Fin} : fin de la recherche (\MontreCode{\textbackslash pflxmax} par défaut).
\end{itemize}
...
\begin{GraphiqueTikz}[Xmin=-5,Xmax=5,Ymin=-2,Ymax=5]
  ...
  \GenererPolynomeLagrange{-4,-3,0,2,4}{-1,3.5,2,1,0}
  \TracerCourbe[Couleur=red,Nom=cf]{polylagrange(x)}
  \TrouverPointsInteret%
    [%
      Racines=true,Maximum=true,Minimum=false,%
      CouleurMax=blue!70!black,CouleurMin=red,CouleurRacines=orange%
    ]%
    {cf}
  %récupération des coordonnées
  \RecupererCoordonnees{(c-max)}[\MonMaxX][\MonMaxY]
\end{GraphiqueTikz}

Le maximum est $M\approx\ArrondirNum[2]{\MonMaxY}$, atteint en $x\approx\ArrondirNum[2]{\MonMaxX}$
\end{tcblisting}
...
\begin{tcblisting}{listing engine=minted,minted language=latex,colframe=lightgray,colback=lightgray!5,listing only}
%usepackage{csvsimple}
\TracerNuageDepuisCSV[Clés](Séparateur)<Clés points>{fichier.csv}{colX}{colY}
\end{tcblisting}
...
\begin{filecontents*}[overwrite]{datatestgrapheur.csv}
n;x;y
1;10;245
2;20;189
3;30;142
...
\begin{tcblisting}{listing engine=minted,minted language=latex,colframe=lightgray,colback=lightgray!5,listing only}
\CreerListesDepuisCSV[clés](Séparateur){fichier.csv}{colX}{colY}{nomMacroX}{nomMacroY}
\end{tcblisting}
...
\begin{itemize}
  ...
  \item mêmes clés \MontreCode{ModeTransfo}, \MontreCode{MaxLignes} que \MontreCode{\textbackslash TracerNuageDepuisCSV}.
\end{itemize}
...
\subsection{Mini graphique \og clé en main \fg{}}\label{minigraphique}

Pour illustrer rapidement une ou plusieurs fonctions (typiquement pour un tableau de variations, une correction, ou une vignette), une commande dédiée permet d'obtenir un petit graphique \textit{sans passer par l'environnement} \MontreCode{GraphiqueTikz} : pas besoin de définir la fenêtre, les axes ou la grille séparément.

\begin{tcblisting}{listing engine=minted,minted language=latex,colframe=lightgray,colback=lightgray!5,listing only}
  \MiniGraphiqueTkz[clés tikz]<clés>{fct1§fct2§...}[VI1§VI2§...]
\end{tcblisting}

Le premier argument (optionnel) contient des options \TikZ{} classiques, le second (entre \MontreCode{<...>}) les \MontreCode{clés} ci-dessous. Le troisième argument, obligatoire, contient une (ou plusieurs, séparées par \MontreCode{§}) formule(s) \MontreCode{xint} en \MontreCode{x}. Le quatrième argument, optionnel, contient les valeurs interdites associées à chaque fonction (même syntaxe, séparées par \MontreCode{§}).
...
\begin{itemize}
  \item \MontreCode{Largeur}, \MontreCode{Hauteur} : dimensions du mini graphique (\MontreCode{4cm}/\MontreCode{3cm} par défaut) ;
  \item \MontreCode{Xmin}, \MontreCode{Xmax}, \MontreCode{Ymin}, \MontreCode{Ymax} : fenêtre (\MontreCode{-3}/\MontreCode{3}/\MontreCode{-2}/\MontreCode{2} par défaut) ;
  \item \MontreCode{Couleur} : couleur(s) des courbes (\MontreCode{black} par défaut) ; en cas de plusieurs fonctions, une liste séparée par des \texttt{§} est \textit{cyclée} automatiquement ;
  \item \MontreCode{Domaine} : domaine(s) de tracé sous la forme \MontreCode{a:b} (un par fonction, séparés par \MontreCode{§}) ; par défaut, c'est toute la fenêtre \MontreCode{[Xmin;Xmax]} qui est utilisée ;
  \item \MontreCode{DeltaVI} : pas utilisé autour des valeurs interdites (\MontreCode{auto} par défaut, calculé à partir de la largeur de la fenêtre) ;
  \item \MontreCode{Echelle} : sert à ajuster proportionnellement les épaisseurs de traits (\MontreCode{auto}, basé sur \MontreCode{Hauteur}, ou une valeur numérique) ;
  \item \MontreCode{Cadre} : booléen (\MontreCode{true} par défaut) pour afficher un cadre autour du mini graphique ;
  \item \MontreCode{ProfLycee} : booléen (\MontreCode{false} par défaut) pour un style de traits plus \textit{fin}, cohérent avec les figures du package \textsf{ProfLycee} ;
  \item \MontreCode{MainLevee} : booléen (\MontreCode{false} par défaut, expérimental) pour un rendu \og à main levée \fg{} (axes, cadre et courbes légèrement irréguliers/bruités).
\end{itemize}

\begin{tcblisting}{listing engine=minted,minted language=latex,colframe=lightgray,colback=lightgray!5}
\MiniGraphiqueTkz<Xmin=-3,Xmax=3,Ymin=-2,Ymax=6,Couleur=blue>{x^2-1}
\qquad
\MiniGraphiqueTkz<Xmin=-4,Xmax=4,Ymin=-2,Ymax=2,Couleur=red,MainLevee>{sin(x)}
\end{tcblisting}

Plusieurs courbes peuvent être tracées dans le même mini graphique, avec un domaine et une valeur interdite propres à chacune :

\begin{tcblisting}{listing engine=minted,minted language=latex,colframe=lightgray,colback=lightgray!5}
\MiniGraphiqueTkz%
  <Largeur=5cm,Hauteur=4cm,Xmin=-1,Xmax=6,Ymin=-1,Ymax=7,%
  Couleur={blue§red},Domaine={0:6§0.2:6}>%
  {sqrt(x)§1/x+1}[{}§0]
\end{tcblisting}

Enfin, une variante \MontreCode{\textbackslash MiniGraphiqueTkzTab} permet d'insérer directement un mini graphique dans une ligne d'un tableau de variations \textsf{tkz-tab} :

\begin{tcblisting}{listing engine=minted,minted language=latex,colframe=lightgray,colback=lightgray!5,listing only}
%dans un environnement tkzTabInit
\MiniGraphiqueTkzTab%
  [clés minigraphique tikz]<clés minigraphique>%
  {fct}[VI]{numéro de ligne}%
  [échelle (0.85)][Décalage (1.5)]
\end{tcblisting}

Les deux derniers arguments optionnels ajustent respectivement la taille (\MontreCode{0.85} par défaut) et le décalage horizontal (\MontreCode{1.5} par défaut) du mini graphique par rapport au tableau.
...
\begin{description}[noitemsep]
  ...
  \lstcmd{grads auto~~~~~}{[GradsAuto=...]}{gradsauto}
  ...
  \lstcmd{mini graphique~}{\MiniGraphiqueTkz}{minigraphique}
  \lstcmd{~~~~~~~~~~~~~~~}{\MiniGraphiqueTkzTab}{minigraphique}
\end{description}
...
\begin{description}[noitemsep]
  ...
  \lstcmd{~~~~~~~~~~~~~~~}{\TracerNuageSuiteF}{nuagesuite}
  \lstcmd{~~~~~~~~~~~~~~~}{\TracerNuageSuiteFn}{nuagesuite}
  \lstcmd{~~~~~~~~~~~~~~~}{\TracerNuageSuiteAuto}{nuagesuite}
...
\begin{description}[noitemsep]
  ...
  \lstcmd{points intérêt~}{\TrouverPointsInteret}{pointsinteret}
...
\begin{description}[noitemsep]
  ...
  \lstcmd{~~~~~~~~~~~~~~~}{\RepresenterIntegrale}{integrnew}
...
\begin{verbatim}
0.31e : Améliorations dans les codes de la version normalisation
0.31d : Wrapper complet pour les suites récurrentes (\TracerNuageSuiteAuto)
-----   Graduations automatiques 'nice numbers' (GradsAuto)
-----   Mini graphique 'clé en main' (\MiniGraphiqueTkz)
0.31c : Améliorations dans les codes (suites notamment)
0.31b : Améliorations dans les codes
